...
namespace BH.Framework.Infrastructure.Resource.Services
{
    ...
    public class AddressableService : IAddressableService
    {
        ...
        [Inject]
        public AddressableService(ILogService logService)
        {
            _logService = logService;
            _logService.Info("AddressableService 开始初始化...");
        }
        ...
        public async Task<T> LoadAssetAsync<T>(string address) where T : class
        {
            ...
            {
                _logService.Error($"[AddressableService] 加载失败: 地址为空或null。");
                return null;
            }
            ...
            try
            {
                ...
                {
                    _logService.Warning($"[AddressableService] 资源 '{address}' 已经被加载，正在从缓存中获取。");
                ...
                if (resultAsset != null)
                {
                    ...
                    _logService.Info($"[AddressableService] 成功加载资源: {address} (Type: {typeof(T)})");
                    return resultAsset;
                }
                else
                {
            ...
            {
                _logService.Error($"[AddressableService] 加载资源时发生异常: {address}\n{ex.Message}\n{ex.StackTrace}");
                return null;
            }
        }
            ...
            bool instantiateInWorldSpace = false)
        {
            ...
            if (!prefab)
            {
                _logService.Error($"[AddressableService] 无法实例化预制体，因为加载失败: {address}");
                return null;
            }

            var instance = Object.Instantiate(prefab, parent, instantiateInWorldSpace);
            _logService.Info($"[AddressableService] 成功实例化预制体: {address}");
            return instance;
        }
        ...
        public void ReleaseAsset(string address)
        {
            if (string.IsNullOrEmpty(address))
            {
                _logService.Error("[AddressableService] 释放失败: 地址为空或null。");
                return;
            }
            ...
            if (_loadedHandles.TryGetValue(address, out AsyncOperationHandle handle))
            {
                ...
                _logService.Info($"[AddressableService] 已释放资源: {address}");
            }
            else
            {
        ...
        public void ReleaseAllAssets()
        {
            foreach (var pair in _loadedHandles)
            {
                ...
                _logService.Info($"[AddressableService] 已释放资源: {pair.Key}");
            }

            _loadedHandles.Clear();
            _logService.Info("[AddressableService] 已清空所有资源引用。");
        }
